...
const componentRootPath = 'src/layout';
const componentSecondRootPath = 'src/components';
...
function generateComponentFactory() {
  ...
  componentFiles.forEach((componentFile) => {
    ...
    imports.push(
      `import ${importVarName} from '../layout/${componentFile
  ...
  const componentSecondaryFiles = extractVueFiles(componentSecondRootPath);
  componentSecondaryFiles.forEach((componentFile) => {
    if (!fs.existsSync(componentFile)) return;
    ...
    imports.push(
      `import ${importVarName} from '../components/${componentFile
        .replace(path.join(componentSecondRootPath, '/'), '')
        .replace(/\\/g, '/')
        .replace('.vue', '')}';`
    );
    registrations.push(`components.set('${componentName}', ${importVarName});`);
