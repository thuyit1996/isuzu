...
const vueConfig = {
  ...
  chainWebpack: (config) => {
	...
	config.plugin('copy').tap(webpackCopyPluginCustom);
  },
};
// Modify the `CopyWebpackPlugin` to _not_ copy the `public/index.html` to `dist/index.html`
// when building the server bundle.
function webpackCopyPluginCustom(pluginArgs) {
  if (!pluginArgs || !Array.isArray(pluginArgs) || pluginArgs.length <= 0) {
    return pluginArgs;
  }

  // CopyWebpackPlugin args are received in chain webpack as an array, e.g. new CopyWebpackPlugin(arg0) would be received as: `[arg0]`
  // However, the first argument is an array containing the "actual" arguments passed to the plugin, e.g.
  /*
        [
          {
            from: 'E:\\DEV\\Altola\\Support\\jss\\blee\\public',
            to: 'E:\\DEV\\Altola\\Support\\jss\\blee\\dist',
            toType: 'dir',
            ignore: [
              '.DS_Store'
            ]
  ...
  // So, get the first argument, then iterate the elements of that array.
  // Each element defines a `from` and `to`, and optionally a `toType` and `ignore` property.
  pluginArgs[0].forEach((arg) => {
    // We want to exclude the `index.html` file from being copied as part of a directory copy.
    if (arg.toType === 'dir') {
      // The `ignore` property may not be defined (if other webpack customizations have been made),
      // so need some conditional checks around that scenario.
      if (arg.ignore && Array.isArray(arg.ignore)) {
        arg.ignore.push('index.html');
      } else {
        arg.ignore = ['index.html'];
      }
    }
  });
