<template>
  <div class="form-group col-md-6" :class="this.fields.cssClass">
    ...
    <input :id="fields.itemId" type='text' @input="inputChange" v-model='valueInput' :name="fields.name" :placeholder="fields.placeholderText" :class="errorMessages && errorMessages.length ? 'input-error' : ''">
    ...
    <div :key="index" v-for="(message, index) in errorMessages">
      <div class="error-invalid" v-html="replaceEnterCharacter(message)"></div>
    </div>
    <div></div>
  </div>
</template>
